class PathConverter
  ...
  def to_dir(dest:)
    ...
    units.each_with_index do |unit, unit_i|
      unit_folder = File.join(@destination, "#{unit_i + 1}_#{unit.title.sterilize}")
      ...
      (unit.children || []).each_with_index do |item, item_i|
        ...
        if item.attachments.length > 0
          item_folder = File.join(unit_folder, "#{item_i + 1}_#{item.title.sterilize}")
          ...
          File.write(File.join(unit_folder, "#{item_i + 1}_#{item.title.sterilize}.md"), @md.body)
        end
      end

      File.write("#{unit_folder}/meta.json", JSON.pretty_generate(unit.to_h))
    end
